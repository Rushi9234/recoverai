...
import {
  ...
  Play, RefreshCw, Lock, AlertTriangle, FileText, Activity
} from "lucide-react";
...
export default function CaseDetailTrace() {
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="flex items-center space-x-3 text-gray-400 font-mono text-sm">
          ...
          <span>Loading Agent Trace for Case {caseId}...</span>
        </div>
      </div>
    );
  }
  ...
  const policyBlocked = policyDecision === "BLOCK";
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between pb-4 border-b border-gray-800">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">Customer: <span className="text-gray-200 font-semibold">{customer.name}</span> | Sub: <span className="font-mono text-blue-400">{subscription.external_ref || "sub_demo"}</span></p>
          </div>
        </div>
      ...
      {/* Hero Visual Chain Header */}
      <div className="p-6 bg-[#0d1322] border border-blue-900/60 rounded-xl space-y-4 shadow-xl">
        <div className="flex items-center justify-between">
          <h2 className="text-xs font-mono font-bold tracking-wider text-blue-400 uppercase">AUDITABLE RECOVERY VISUAL CHAIN</h2>
          <span className="text-[10px] font-mono text-gray-400">RULE: AI PROPOSES → POLICY DECIDES → EXECUTOR ACTS</span>
        </div>

        {/* 5-Step Visual Chain */}
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
          {/* Step 1: AI Proposes */}
          <div className="p-3 bg-[#111827] border border-gray-800 rounded-lg space-y-2">
            ...
            <div className="text-xs font-mono font-bold text-blue-400">{recommendation.action || "RETRY_LATER"}</div>
            <div className="text-[10px] text-gray-400 truncate">Conf: {(diagnosis.confidence * 100).toFixed(0)}%</div>
          </div>

          {/* Step 2: Policy Check */}
          <div className={`p-3 bg-[#111827] border rounded-lg space-y-2 ${policyBlocked ? "border-red-800" : "border-gray-800"}`}>
            <div className="flex items-center justify-between text-[11px] font-mono font-bold text-gray-400">
              <span>2. POLICY CHECK</span>
              <ShieldCheck className={`w-3.5 h-3.5 ${policyBlocked ? "text-red-400" : "text-emerald-400"}`} />
            </div>
            <div className="text-xs font-mono font-bold">
              <StatusBadge status={policyDecision} type="decision" />
            </div>
            <div className="text-[10px] text-gray-400 truncate">{policyPrev.reason || "All checks evaluated"}</div>
          </div>
          ...
          <div className="p-3 bg-[#111827] border border-gray-800 rounded-lg space-y-2">
            ...
            <div className="text-xs font-mono font-bold text-purple-300">
              {actions.length > 0 ? actions[0].status : "READY"}
            </div>
            <div className="text-[10px] text-gray-400 font-mono">Mode: {execMode}</div>
          </div>
          ...
          <div className="p-3 bg-[#111827] border border-gray-800 rounded-lg space-y-2">
            ...
            <div className="text-xs font-mono font-bold text-emerald-400">
              {actions.length > 0 ? formatCurrency(actions[0].outcome_amount_minor) : "₹0"}
            </div>
            <div className="text-[10px] text-gray-400 truncate">Result Recorded</div>
          </div>

          {/* Step 5: Audit */}
          <div className="p-3 bg-[#111827] border border-gray-800 rounded-lg space-y-2">
            <div className="flex items-center justify-between text-[11px] font-mono font-bold text-gray-400">
              <span>5. AUDIT PROOF</span>
            ...
            <div className="text-[10px] text-gray-400 truncate">{auditEvents.length} events logged</div>
          </div>
        </div>
      </div>
      ...
      {/* Action Messages */}
      {actionError && (
        <div className="p-4 bg-red-950/80 border border-red-800 rounded-xl text-red-300 text-xs font-mono flex items-center space-x-2">
          <XCircle className="w-4 h-4 text-red-400 shrink-0" />
          <span>{actionError}</span>
        </div>
      )}
      {actionSuccess && (
        <div className="p-4 bg-emerald-950/80 border border-emerald-800 rounded-xl text-emerald-300 text-xs font-mono flex items-center space-x-2">
        ...
        <div className="space-y-6">
          ...
          <div className="p-5 bg-[#111827] border border-gray-800 rounded-xl space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs font-mono text-blue-400 font-bold">{(diagnosis.confidence * 100).toFixed(0)}% CONFIDENCE</span>
            </div>
